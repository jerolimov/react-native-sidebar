...
// API test / prototyping... Some methods will be removed later again.
	...
	_handleMove(event, gestureState) {
		const currentProgress = this.progressOffset + gestureState.dx * this.progressFactor;
	...
	_handleMoveEnd(event, gestureState) {
		...
		let toValue = 0;
		if (this.props.leftSidebar && currentProgress + gestureState.vx >= 0.5) {
			toValue = 1;
		} else if (this.props.rightSidebar && currentProgress + gestureState.vx < -0.5) {
			toValue = -1;
		}
